    ...
    ui(new Ui::Ranking)
{
    ...
    for(QJsonObject::iterator  p = total.begin();p!=total.end();p++){
        ...
        pair<QString,int>* val2 = new pair<QString,int>(o["name"].toString(),o["capacity"].toInt());
        pair<pair<int,int>,pair<QString,int>>* v = new pair<pair<int,int>,pair<QString,int>>(*val,*val2);
        sort.insert(*v);
    }
    ...
    Row* r = new Row;
    multimap<pair<int,int>,pair<QString,int>>::iterator  p = sort.begin();
    for(int i=sort.size();p!=sort.end();p++,i--){
       pair<pair<int,int>,pair<QString,int>> value = *p;
       r->add(value.first.first,i,value.first.second,value.second.first,data->getName(),value.second.second);
    }
...
void Row::add(int level_, int rank_, int exp_, QString name_, QString gold, int cpc){
    rank = new QLabel;
    h = new QHBoxLayout;
